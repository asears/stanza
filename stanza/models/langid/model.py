...
class LangIDBiLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.char_embeds = nn.Embedding(
            num_embeddings=self.vocab_size, embedding_dim=self.embedding_dim, padding_idx=self.padding_idx
        )

        # the bidirectional LSTM
        self.lstm = nn.LSTM(
            self.embedding_dim, self.hidden_dim, num_layers=self.num_layers, bidirectional=True, batch_first=True
        )
    ...
    def build_lang_mask(self, use_gpu=None):
        ...
        lang_mask_list = (
            [int(lang in self.lang_subset) for lang in self.idx_to_tag]
            if self.lang_subset
            else [1 for lang in self.idx_to_tag]
        )
