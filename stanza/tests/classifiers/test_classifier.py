...
class TestClassifier:
    ...
    def test_train_bert(self, tmp_path, fake_embeddings, train_file, dev_file):
        ...
        saved_model = torch.load(save_filename, lambda storage, _loc: storage, weights_only=True)
    ...
    def test_finetune_bert_layers(self, tmp_path, fake_embeddings, train_file, dev_file):
        ...
        initial_model = torch.load(initial_model, lambda storage, _loc: storage, weights_only=True)

        second_model_file = list(save_path.glob("*E0002*"))
        assert len(second_model_file) == 1
        second_model_file = second_model_file[0]
        second_model = torch.load(second_model_file, lambda storage, _loc: storage, weights_only=True)
        ...
        final_model = torch.load(fifth_model_file[0], lambda storage, _loc: storage, weights_only=True)
        for layer_idx in range(2):
    ...
    def test_finetune_peft(self, tmp_path, fake_embeddings, train_file, dev_file):
        ...
        # anything correct
        pipeline = stanza.Pipeline("en", download_method=None, model_dir=str(TEST_MODELS_DIR), processors="tokenize,sentiment", sentiment_model_path=save_filename, sentiment_pretrain_path=str(fake_embeddings))
    ...
    def test_finetune_peft_restart(self, tmp_path, fake_embeddings, train_file, dev_file):
        ...
        saved_model = torch.load(save_file, lambda storage, _loc: storage, weights_only=True)
        assert any(x.find(".encoder.") >= 0 for x in saved_model['params']['bert_lora'])
        ...
        initial_model = torch.load(initial_model_file, lambda storage, _loc: storage, weights_only=True)
        ...
        final_model = torch.load(final_model_file, lambda storage, _loc: storage, weights_only=True)
