...
def build_default_config(resources, lang, model_dir, load_list):
    ...
    for item in load_list:
        ...
        # handle case when identity is specified as lemmatizer
            ...
            default_config[f"{processor}_model_path"] = os.path.join(model_dir, lang, processor, package + '.pt')
...
def get_root_from_zipfile(filename):
    ...
    assert len(zf.filelist) > 0, f"Zip file at f{filename} seems to be corrupted. Please check it."
    ...
    if processors:
        ...
        for key, value in processors.items():
            ...
            # allow processors to be set to variants that we didn't implement
                ...
                logger.debug(f'Found {key}: {value}. ' f'Using external {value} variant for the {key} processor.')
                processor_list[key] = value
            ...
            # not a processor in the officially supported processor list
                ...
                logger.debug(f'{key}: {value} is not officially supported by Stanza, ' f'loading it anyway.')
                processor_list[key] = value
            # cannot find the package for a processor and warn user
            else:
                logger.warning(f'Can not find {key}: {value} from official model list. ' f'Ignoring it.')
    # resolve package
    if package:
            ...
            for key in PIPELINE_NAMES:
                ...
                if package in resources[lang][key]:
                    ...
                    if key not in processor_list:
                        ...
                        logger.debug(f'{key}: {package} is overwritten by ' f'{key}: {processors[key]}.')
            if not flag:
                logger.warning((f'Can not find package: {package}.'))
...
def set_logging_level(logging_level, verbose):
    ...
    if logging_level not in all_levels:
        raise ValueError(
            f"Unrecognized logging level for pipeline: " f"{logging_level}. Must be one of {', '.join(all_levels)}."
        )
...
def process_pipeline_parameters(lang, model_dir, package, processors):
    ...
    if isinstance(lang, str):
        ...
        raise TypeError(f"The parameter 'lang' should be str, " f"but got {type(lang).__name__} instead.")

    if isinstance(model_dir, str):
        model_dir = model_dir.strip()
    elif model_dir is not None:
        raise TypeError(f"The parameter 'model_dir' should be str, " f"but got {type(model_dir).__name__} instead.")

    if isinstance(package, str):
        package = package.strip().lower()
    elif package is not None:
        raise TypeError(f"The parameter 'package' should be str, " f"but got {type(package).__name__} instead.")
    ...
    if isinstance(processors, str):
        ...
        raise TypeError(
            f"The parameter 'processors' should be dict or str, " f"but got {type(processors).__name__} instead."
        )

    return lang, model_dir, package, processors


def download_resources_json(model_dir, resources_url, resources_branch, resources_version, proxies=None):
    """
...
def list_available_languages(
    ...
    languages = [lang for lang in resources if not isinstance(resources[lang], str) and 'alias' not in resources[lang]]
...
def download(
    ...
    lang, model_dir, package, processors = process_pipeline_parameters(lang, model_dir, package, processors)
    ...
    if package == 'default' and (processors is None or len(processors) == 0):
        ...
        request_file(
            ...
            os.path.join(model_dir, lang, 'default.zip'),
            proxies,
    ...
    # Customize: maintain download list
        ...
        logger.info(
            f'Downloading these customized packages for language: ' f'{lang} ({lang_name})...\n{download_table}'
        )
