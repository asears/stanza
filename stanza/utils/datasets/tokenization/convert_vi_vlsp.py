...
def find_spaces(sentence: List[str]) -> List[str]:
    """Find spaces in a sentence."""
    # TODO(John Bauer): there are some sentences where there is only one quote,
...
def add_vlsp_args(parser) -> None:
    parser.add_argument(
        '--include_pos_data',
        action='store_true',
        default=False,
        help='To include or not POS training dataset for tokenization training. The path to POS dataset is expected to be in the same dir with WS path. For example, extern_dir/vietnamese/VLSP2013-POS-data',
    )
    parser.add_argument(
        '--vlsp_include_spaces',
        action='store_true',
        default=False,
        help='When processing vi_vlsp tokenization, include all of the spaces.  Otherwise, we try to turn the text back into standard text',
    )


def write_file(vlsp_include_spaces, output_filename, sentences: List[str], shard) -> None:
...
def convert_pos_dataset(file_path) -> List[str]:
    """
...
def convert_file(
    vlsp_include_spaces, input_filename, output_filename, shard, split_filename=None, split_shard=None, pos_data=None
) -> None:
...
def convert_vi_vlsp(extern_dir, tokenizer_dir, args) -> None:
    ...
    if args.include_pos_data:
        if not os.path.exists(input_pos_filename):
            raise FileNotFoundError("Cannot find pos dataset for VLSP at %s" % input_pos_filename)
        else:
    ...
    convert_file(
        args.vlsp_include_spaces,
        input_train_filename,
