...
python3 process_sb10k.py --csv_filename train.csv --out_dir out --sentiment_column 1 --text_column 4 --split Split.TRAIN_DEV_TEST
...
parser.add_argument(
    '--split', type=lambda x: Split[x.upper()], default=Split.TRAIN_DEV_TEST, help="How to split the resulting data"
)
...
if args.split is Split.TRAIN_DEV_TEST:
    ...
    process_utils.write_splits(
        args.out_dir, snippets, (process_utils.Split("train.txt", 0.9), process_utils.Split("dev.txt", 0.1))
    )
elif args.split is Split.TEST:
    process_utils.write_list(os.path.join(args.out_dir, "test.txt"), snippets)
else:
    raise ValueError("Unknown split method {0}".format(args.split))
